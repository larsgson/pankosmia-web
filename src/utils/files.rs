use crate::structs::{AppSettings, PankosmiaError, TypographyFeature, UserSettings};
use crate::utils::client::Clients;
use crate::utils::paths::{
    home_dir_string, maybe_os_quoted_path_str, os_slash_str, user_settings_path,
};
use rocket::State;
...
pub(crate) fn customize_and_copy_template_file(
    from_path: &str,
    to_path: &String,
    working_dir: &String,
    app_resources_dir: &String,
    pankosmia_dir: &String,
) -> Result<(), std::io::Error> {
    let json_string = fs::read_to_string(&from_path)?;
    let quoted_json_string = maybe_os_quoted_path_str(
        json_string
...
pub(crate) fn load_and_substitute_json(
    from_path: &str,
    from_text: &str,
    to_text: &str,
...
pub(crate) fn write_user_settings(
...
pub(crate) fn copy_and_customize_webfont_css(
    ...
    let feature_json = match &features_json[font_name] {
        ...
        _ => {
            return Err(PankosmiaError(format!(
                "No feature data in user_settings for font {}",
                font_name
            )))
        }
    };
    let source_font_file_path = format!(
        "{}{}pankosmia-{}.css",
        &template_path,
        os_slash_str(),
        &font_name
    );
    let target_font_file_path = format!(
        "{}{}pankosmia-{}.css",
        &target_path,
        os_slash_str(),
        &font_name
    ...
    if Path::new(&source_font_file_path).is_file() {
        ...
        for feature_pair in feature_json {
            font_feature_collector.push(format!(
                "{}: {}",
                feature_pair["key"], feature_pair["value"]
            ));
        }
        ...
        match fs::write(&target_font_file_path, css_string) {
            ...
            Err(e) => {
                panic!(
                    "Could not write css file '{}': {}",
                    target_font_file_path, e
                );
            }
        }
    } else {
        Err(PankosmiaError(format!(
            "Could not find source font file '{}'",
            source_font_file_path
        )))
    }
}

pub(crate) fn copy_and_customize_webfont_css2(
    template_path: &String,
    target_path: &String,
    font_features: &Vec<TypographyFeature>,
    font_name: &String,
) -> Result<(), PankosmiaError> {
    let source_font_file_path = format!(
        "{}{}pankosmia-{}.css",
