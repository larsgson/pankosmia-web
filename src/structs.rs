...
pub struct AppSettings {
    ...
    /// The fallback language used by per-language endpoints when no
    /// `X-Language-Code` header / `LanguageContext` (M5+) resolves a
    /// specific one. Single-tenant FS deployments use this for every
    /// per-language operation. Defaults to `"en"` if not configured.
    #[serde(skip, default = "default_language_en")]
    pub default_language: crate::identity::LanguageCode,
}

fn default_language_en() -> crate::identity::LanguageCode {
    crate::identity::LanguageCode::parse("en").unwrap()
}
